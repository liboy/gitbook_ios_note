...
- _commonModes：一个 mode 可以标记为 `common` 属性用 CFRunLoopAddCommonMode函数它就会保存在 commonModes。主线程 CommonModes 默认已有两个modek CFRunLoopDefaultMode 和 UITrackingRunLoopMode，当然你也可以通过调用 CFRunLoopAddCommonMode() 方法将自定义mode 放到 kCFRunLoopCommonModes 组合）。
