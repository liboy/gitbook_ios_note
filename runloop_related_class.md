...
```
- CommonModes：一个 Mode 可以将自己标记为`Common`属性（通过将其 ModeName 添加到 RunLoop 的 `commonModes` 中）,当 RunLoop 的内容发生变化时，RunLoop 都会自动将 _commonModeItems 里的 Source/Observer/Timer 同步到具有 `Common` 标记的所有Mode里。
